...
class ICal {
    ...
    public function del_export($del_arr, $name, $calendar) {
        $this->name = $name;
        $this->data ="BEGIN:VCALENDAR
CALSCALE:GREGORIAN
X-WR-RELCALID:RCD Calendar ".$calendar."
X-WR-CALNAME:RCD Calendar ".$calendar."
...
METHOD:CANCEL";
        ...
        foreach ($del_arr as $key => $value) {
           ...
           $this->data .=  "
BEGIN:VEVENT
UID:" .$value['booking_number'] . "
SUMMARY:Not available
DTSTART;VALUE=DATE:" . date("Ymd", strtotime($value['date_from'])) . "
DTEND;VALUE=DATE:" . date("Ymd", strtotime($value['date_to'])) . "
STATUS:CANCELLED
END:VEVENT";
        }
...
END:VCALENDAR";
    ...
    public function export($arr, $del_arr, $name, $calendar) {
        $this->name = $name;
        $this->data ="BEGIN:VCALENDAR
CALSCALE:GREGORIAN
METHOD:PUBLISH
...
DTSTART;VALUE=DATE:" . date("Ymd", strtotime($value['date_from'])) . "
DTEND;VALUE=DATE:" . date("Ymd", strtotime($value['date_to'])) . "
DTSTAMP;VALUE=DATE-TIME:".date("Ymd", time())."T141053Z
DESCRIPTION:Occupied from " . date("d.m.Y", strtotime($value['date_from'])) . " to " . date("d.m.Y", strtotime($value['date_to'])) . "
END:VEVENT";
        }
...
END:VCALENDAR";
    ...
    public function save($id) {
        ...
        $file = INSTALL_PATH . 'ical/' . $id . '/export.ics';

        if (!file_exists($file)) {
            mkdir(INSTALL_PATH . 'ical/' . $id, 0777, true);
        }else{
            unlink($file);
        }
        $h = fopen($file, 'w+');
        ...
        /*  $url = $this->name;
          ...
          $GlobalFileHandle = fopen(INSTALL_PATH . "tmp.ics", 'w+');
    ...
    public function del_save($id) {
        ...
        $file = INSTALL_PATH . 'ical/' . $id . '/deleted.ics';
